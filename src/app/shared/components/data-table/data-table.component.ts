...
export class DataTableComponent implements OnInit, OnChanges {
  ...
  displayDataTable(): void {
    this.setId = this.id !== undefined ? this.id : 'gridtable';
    const noData = this.data.result.length > 0 ? '' : 'no-data-available';
    ...
    if (this.setId === 'RawkeysGrid') {
      this.removeButtonID = '#removerawkeysBtn';
    } else if (this.setId === 'LookupkeysGrid') {
      this.removeButtonID = '#removelookupkeysBtn';
    } else {
      this.removeButtonID = this.removeButtonID;
    }
    ...
    headers.forEach((data, index) => {
      ...
      if (data.isCheckbox) {
        def['render'] = (dat, type, full, meta) => {
          let disableClass = full.isDefault ? 'disabled' : '';
          ...
          return '<input type="hidden" id="' + data.data + '" name="' + data.data + '" value="' + dat +
      ...
      if (opt.isEditOption && data.editButton) {
        def['render'] = (dat, type, full, meta) => {
          if (opt.isRowTitle) {
            return '<dtext title="' + dat + '">' + dat + '</dtext><i class="fa fa-pencil" aria-hidden="true"></i>';
          } else {
    ...
    if (!_.isUndefined(option.isSortingColumn)) {
      sortingColumn = [[option.isSortingColumn, 'asc']];
    }
    ...
    this.tableWidget[this.tableId] = table.DataTable({
      ...
      rowCallback: (row, data, index) => {
        ...
        } else if (!_.isUndefined(_.find(comThis.data.headers, o => o.data === 'Value'))) {
          const headerObject = _.find(comThis.data.headers, o => o.data === 'Value');
          const indexVal = _.findIndex(comThis.data.headers, headerObj => _.includes(headerObj.fieldType, 'list'));
          if (indexVal > 0) {
    ...
    table.on('change', '.switch input', (e: any) => {
      if ($(e.currentTarget).prop('checked')) {
        $(e.currentTarget).next('.slider').removeClass('in-active').addClass('a-tive');
      } else {
        $(e.currentTarget).next('.slider').removeClass('a-tive').addClass('in-active');
      }
      if (_.includes(_.map(comThis.data.headers, 'onloadField'), true)) {
        const obj = {};
    ...
    table.on('change', 'select.globalParameters', function () {
      ...
      if (elementID === 'Type') {
        if (!_.isUndefined(comThis.data.parametersType)) {
        ...
        $(this).closest('tr').remove();
        const parameterToPass = comThis.data.parametersType === 'organization' ?
          comThis.data.globalParameters : comThis.data.organizationParameters;
    ...
    function editRowRender() {
      ...
      $.each($tds, function (i, el) {
        ...
        const maxTxt = !_.isUndefined(headers[hi].maxLength) ? headers[hi].maxLength : '524288';
        const minTxt = !_.isUndefined(headers[hi].minLength) ? headers[hi].minLength : '0';
        const initialData = _.filter(comThis.data.result, v => v.id === $(this).parent().find('input').eq(0).val())[0] || [];
      ...
      $.each($tds, (i: any, event: any) => {
        if (_.has($(event).find('input,select')['0'].attributes, ['data-index']) ||
          ...
          if (_.isEqual(initialRowData[headers[i].data], 'list')) {
            ...
            if (option.isRowTitle) {
              $(event).parent().find('td').eq(i + 1).html('<dtext title="' + txt.toString() + '">' + txt.toString() + '</dtext>');
            } else {
    ...
    table.on('mousedown.save', 'button.btn-save', (e: any) => {
      ...
      $.each($tds, (i: any, event: any) => {
        ...
        if (_.includes(headMap, $(event).find('input,select').attr('id'))) {
          ...
          const tagOptions = _.filter(comThis.data.result, v => v.id === $(event).parent().find('input').eq(0).val())[0] || [];
    ...
    table.on('change', 'tr td:first-of-type input', (event: any) => {
      ...
      if (!$(event.currentTarget).prop('checked') && !_.includes(arrayPush, undefined)) {
        ...
        arrayPush = _.remove(arrayPush, v => v[comThis.data.headers[0].data] !== itemToRemove);
        table.find('tr th:first-of-type input').prop('checked', false);
        $(comThis.removeButtonID).addClass('disabled');
      } else {
        const undeJs = {};
        let rmvFilter = _.filter(comThis.data.result, [comThis.data.headers[0].data, $(event.currentTarget).val()])[0];
        if (_.size(rmvFilter) === 0) {
          undeJs[$(event.currentTarget).attr('name').substring(0, $(event.currentTarget).attr('name').length - 1)] =
            $(event.currentTarget).val();
          undeJs[$(event.currentTarget).attr('id')] = true;
        }
      ...
      if (arrayPush.length === result.length) {
        ...
        if (!$(event.currentTarget).prop('checked')) {
          ...
          $.each(rows, (i: any, e: any) => {
            if (!$(e).find('td:eq(0) input[type="checkbox"]').prop('disabled')) {
              ...
              const undeJs = {};
              let rmvFilter = _.filter(comThis.data.result, [comThis.data.headers[0].data, val])[0];
              if (_.size(rmvFilter) === 0) {
      ...
      $.each($tds, (i: any, event: any) => {
        ...
        if (_.includes(headMap, $(event).find('input,select').attr('id'))
          ...
          if (!_.isEmpty(inputValue)) {
            ...
            updatedRow['Value'] = _.find(comThis.data.result, function (row) { return row.id === updatedRow['id']; }).Value;
          } else if (comThis.data.parametersType === 'organization' && comThis.addButtonID['id'] === 'inheritRow') {
            updatedRow['Value'] = _.find(comThis.data.globalParameters, { Label: updatedRow['Label'] }).Value;
          } else if (comThis.data.parametersType === 'partner' && comThis.addButtonID['id'] === 'inheritRow') {
            updatedRow['Value'] = _.find(comThis.data.organizationParameters, { Label: updatedRow['Label'] }).Value;
          } else {
            updatedRow['Value'] = [];
          }
        }
    ...
    if (_.includes(_.map(comThis.data.headers, 'onloadField'), true)) {
      const onLoad = _.filter(comThis.data.headers, 'onloadField');
      _.forEach(onLoad, (v, i) => {
        table.find('.accessRights').on('change', 'select', (event: any) => {
          if (event.currentTarget.value !== '') {
            ...
            _.filter(comThis.data.result, (vv, ii) => {
              if (vv['feature'] === txt) {
  ...
  rolesactionSection() {
    let data = '';
    if (_.includes(_.map(this.data.headers, 'onloadField'), true) && this.onRoleFieldData.length > 0) {
  ...
  activeInactive(ev) {
    const data = _.filter(this.data.result, (v, i) => v[this.data.headers[0].data] === ev.target.value)[0];
  ...
  createNewRow(event: any, status: any) {
    ...
    // column visibility
    _.forEach(this.data.headers, (v, i) => {
   ...
   */
  removeData(data) {
    const inputChange = { remove: data, functionRef: 'removeRow' };
    this.filterValues.emit(inputChange);
  }
