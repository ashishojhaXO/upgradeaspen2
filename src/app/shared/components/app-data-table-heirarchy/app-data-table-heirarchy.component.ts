...
import {DataTableAction} from './../app-data-table/data-table-action';
import {Router, ActivatedRoute} from '@angular/router';
import {DataTableActionType} from './../app-data-table/data-table-action-type';
...
@Component({
  selector: 'app-data-table-heirarchy',
  templateUrl: './app-data-table-heirarchy.component.html',
  styleUrls: ['./app-data-table-heirarchy.component.scss'],
  moduleId: module.id
})
export class AppDataTableHeirarchyComponent implements OnInit, OnChanges {
  ...
  displayDataTable(): void {
    ...
    headers.forEach((data, index) => {
      ...
      if (data['toggle'] && _.includes(data['data'], 'noDataFeed') && index === 0 ) {
        def['render'] = (dat, type, full, meta) => {
          ...
          const toogleAlert = toogleAction && full['alertEnabled'] ? (full.heirarchyData && full.heirarchyData.length ? '<div style="width: 45px;background: #0777bc;color: #fff;padding: 3px 5px 2px;font-family: Asap Bold;font-size: 10px;float:right"><div style="cursor: pointer" class="fa fa-plus-square-o heirarchy"></div>ALERT</div>' : '<div style="width: 45px;background: #0777bc;color: #fff;padding: 3px 5px 2px;font-family: Asap Bold;font-size: 10px;float:right">ALERT</div>' ) : ( full.heirarchyData && full.heirarchyData.length ? '<div style="width: 45px;float:right; cursor: pointer" class="fa fa-plus-square-o heirarchy"></div>' : '<div style="width: 45px;float:right">&nbsp;</div>');
    ...
    // Add default class to each row
    if($('#schemaGrid tr').length) {
      $('#schemaGrid tr').each(function(){
        $(this).addClass('main');
      });
    }

    // Create Heirarchy Table
    $('#schemaGrid div.heirarchy').on('click', function(item) {
      const trMain = $(this).closest('tr.main');
      const rowIndex = $('#schemaGrid tbody tr.main').index(trMain);
      const dataResult = comThis.dataObject.gridData.result;
      console.log('dasdasda')
      console.log(dataResult[rowIndex]);

      if($(this).hasClass('fa-plus-square-o')) {
        $(this).removeClass('fa-plus-square-o');
        $(this).addClass('fa-minus-square-o');

        if(!trMain.next('tr.hierarchy').length) {

          var objArr = [];

          dataResult[rowIndex].heirarchyData.forEach(function (ele) {
            var obj: any = {};
            obj.name = ele.name;
            obj.frequency = ele.frequency;
            obj.period = comThis.datePipe.transform(ele.report_run_start_time,'MMM-dd-yyyy') + ' - ' +  comThis.datePipe.transform(ele.report_run_end_time, 'MMM-dd-yyyy');
          ...
          trMain.nextUntil('tr.main').removeClass('hidden');
        }
      } else {
        $(this).removeClass('fa-minus-square-o');
        $(this).addClass('fa-plus-square-o');
        trMain.nextUntil('tr.main').addClass('hidden');
      }
    });
