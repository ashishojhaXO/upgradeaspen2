...
@Component({
    ...
    styleUrls: ['./multiselect.component.scss', '../multiselect-dropdown.component.scss'],
...
export class AngularMultiSelect implements OnInit, ControlValueAccessor, OnChanges, Validator, AfterViewChecked, OnDestroy {
    ...
    toggleSelectAll() {
        if (!this.isSelectAll) {
            ...
            this.selectedItems.push.apply(this.selectedItems, this.data);
            this.isSelectAll = true;
            this.onChangeCallback(this.selectedItems);
            this.onTouchedCallback(this.selectedItems);
  ...
  private updateTooltipText() {
    if (this.settings.showTooltip) {
      if (this.selectedItems && this.selectedItems.length > 0 ) {
        const mappedSelectedItems = this.selectedItems
          .slice(0, this.settings.tooltipElementsSize)
          .map( item => item[this.settings.labelKey]);
        this.tooltipText = mappedSelectedItems.join(', ');
        if (this.selectedItems.length > this.settings.tooltipElementsSize) {
          this.tooltipText = this.tooltipText + ' ...more';
        }
      } else {
        this.tooltipText = '';
      }
